...
![post-thumbnail](Airflow.assets/image.png)

## 1.1 Airflow 란?

배치 중심 워크플로우를 개발, 예약 및 모니터링하기 위한 오픈 소스 플랫폼입니다. Airflow의 확장 가능한 Python 프레임워크를 사용하면 거의 모든 기술과 연결되는 워크플로우를 구축할 수 있습니다. 웹 UI는 작업 흐름 상태를 관리하는 데 도움이 됩니다. Airflow는 단일 프로세스부터 가장 큰 워크플로를 지원하는 분산 설정까지 다양한 방식으로 배포할 수 있습니다.

Airflow 프레임워크에는 다양한 기술과 연결하는 연산자가 포함되어 있으며 이를 통해 쉽게 확장할 수 있습니다. 워크플로의 시작과 끝이 명확하고 정기적으로 실행되는 경우 Airflow DAG로 프로그래밍할 수 있습니다.

워크플로는 아래와 같은 특성을 갖는 Python 코드로 정의됩니다.

- 이전 버전으로 롤백할 수 있도록 워크플로우를 원격 저장소에 저장할 수 있습니다.
- 여러 사람이 동시에 워크플로우를 개발할 수 있습니다.
- 기능을 검증하기 위해 테스트코드를 작성할 수 있습니다.
- 구성 요소들은 확장 가능하고 다양한 기존 라이브러리를 기반으로 구축할 수 있습니다.

풍부한 스케줄링 및 실행 체계를 통해 정기적으로 실행되는 복잡한 파이프라인을 쉽게 정의할 수 있으며. 백필(BackFill)을 사용하면 로직을 변경한 후 적재 데이터에 대해 파이프라인을 (재)실행할 수 있습니다. 오류를 해결한 후 부분 파이프라인을 다시 실행하는 기능은 효율성을 극대화하는 데 도움이 됩니다.
...
- **Scalable**
  - Architecture를 어떻게 구성하느냐, Resource가 어느정도 인지에 따라 얼마든지 **Task를 병렬로 실행할 수 있다.**
  - Airflow는 Modular Architecture로 구성되며 **Queue방식을 사용**한다.

- **User Interface**
  - Airflow는 편하고 보기에 좋은 **Web Interface를 제공**한다.
  - Web Appliation을 통해 쉽게 **Pipeline를 모니터링 및 관리**할 수 있다.
...
- **파이썬 기반의 Airfllow는 쉽게 확장이 가능**하고 다양한 시스템과 통합이 가능하다. 실제로 Airflow 커뮤니티에서 **다양한 유형의 데이터 베이스, 클라우드 서비스 등과 통합할 수 있는 수 많은 애드온이 존재**한다
- 수많은 스케줄링 기법은 **파이프라인을 정기적으로 실행**하고 **점진적(증분 : incremental) 처리를 통해 전체 파이프라인을 재실행할 필요 없는** **효율적인 파이프라인 구축**이 가능한다
- **백필** 기능을 사용하면 과거 데이터를 손쉽게 재처리할 수 있기 때문에 코드를 변경한 후 재생성이 필요한 **일괄 데이터 재처리**가 가능한다
- Airflow의 훌륭한 **웹 인터페이스**는 파이프라인 실행 결과를 **모니터링**할 수 있고 오류를 디버깅하기 위한 **편리한 뷰를 제공**한다
...
- 추가 및 삭제 테스트가 빈번한 동적 파이프라인의 경우에는 적합하지 않을 수 있다 (동적 태스크를 구현할 수 있지만, 웹 인터페이스는 DAG의 가장 최근 실행 버전에 대한 정의만 표현, 따라서 **Airflow는 실행되는 동안 구조가 변경되는경우에는 적합하지 않다.**
- **파이썬 언어로만 구현 되어**있고, DAG를 구현하는데까지있어 어느정도의 허들이 존재한다.
...
![](airflow_asset/edge_label_example.png)



![img](airflow_asset/img8.png)
...
- Airflow의 가장 중요한 부분으로, **다양한 DAG Run과 Task Instance 들을 스케쥴링 및 오케스트레이션** 합니다.
- 또한 하나의 DAG Run이 전체 시스템에 크리티컬한 영향을 미치지 않도록 각 DAG Run의 실행 횟수를 제한하기도 합니다.
...
- Sequential, Local, Celery, Kubernetes 등 Executor에는 여러 종류가 있습니다.
...
- **Local Executors** : Task Instance를 Scheduler 프로세스 내부에서 실행합니다.
  ...
  - Local Executor
    ...
    - 익스큐터 내부적으로 워커 프로세스가 FIFO(First in, First out) 알고리즘 적용 방식을 통해 대기열에서 실행할 태스크를 등록합니다.
    - 기본적으로 최대 32개의 병렬 프로세스를 실행합니다.
- **Remote Executors** : Task Instance를 Scheduler 프로세스 외부에서 실행합니다.
  - **Celery Executor**
    ![img](airflow_asset/img7.png)
  ...
  - Kubernetes Executor![img](airflow_asset/img6.png)
    - 쿠버네티스에서 워크로드를 실행합니다.
...
| ------------------ | ------ | ---------------- | ------------------------------------------ |
| SequentialExcutor  | 불가능 | 매우 쉬움        | 시연 / 테스트                              |
...
- DAG, 해당 실행 및 사용자, 역할 및 연결과 같은 기타 Airflow 구성에 대한 메타데이터를 저장합니다.
  ...
  - Airflow를 위한 메타스토어 설정
    ...
    - **워크플로 스크립트** : 스케줄러를 통해 **작업 내역을 분석 및 관리하는 역할**을 수행하며 **메타스토어에 그 해석된 내용을 저장**하는 등의 여러 컴포넌트로 구성되어 있습니다.
...
- 스케줄러에서 분석한 DAG를 시각화하고 DAG 실행과 결과를 확인할 수 있는 주요 인터페이스를 제공합니다
- Airflow의 Web UI 입니다.
...
- Airflow 설치 상태를 전체적으로 확인하고 개별 DAG를 살펴보고 레이아웃, 각 작업 상태 및 각 작업의 로그를 확인하는 가장 좋은 방법입니다.

![](airflow_asset/dags.png)
...
Airflow 는 Single Node, Multi Nodes에서 동작방식이 조금씩 상이하며 각각 어떻게 동작하는지 알아보도록 합니다.
...
**모든 Component는 Metastore와 밀접한 관계를 가지며 동작**한다.



### 1.3.2 **Multi Nodes Architecture**

![img](airflow_asset/img3.png)
...
- **TaskFlow** - `@task`Task로 패키지된 사용자 정의 Python 함수

![img](airflow_asset/img2.png)
...
### 1.4.1 DAG(Directed Acyclic Graph)란?





![img](airflow_asset/img.png)
...
- 만일 두 개의 Operator가 데이터를 공유해서 사용해야 한다면, 하나의 Operator로 합쳐서 사용하기를 권장한다. 그러기 어렵다면 X-COM 이라는 컴포넌트를 사용하여 구성할 수 있다.single task를 정의 해야한다.

- 멱등성을 유지해야한다. 다른 작업간에 겹침 현상을 제거하여 오류제거를 위함이다.
- 자동 재시도를 작성하여 자동으로 재시도 해줄수 있다.
- 하나의 task는 하나의 Operator 클래스로 구성된다.
...
- **두 개의 서로 다른 에어플로우 DAG를 조정해야 하는 경우 ExternalTaskSensor를 사용**하여 다른 DAG의 작업 상태 또는 다른 DAG의 전체 상태를 확인할 수 있음
...
- 여러 개의 Bitshift 연산자를 섞어서 아래와 같이 표현할 수도 있다.
  ...
  ![airflow bitshift](airflow_asset/airflow_bitshift.png)
  ...
  ![airflow bitshift parallel](airflow_asset/airflow_bitshift_parallel.png)
...
- Task-Workflow
  ...
  - removed : 실행이 시작된 후 작업이 DAG에서 사라졌음
    ![image-20231019001618769](airflow_asset/image-20231019001618769.png)
...
```bash
$ kubectl get svc -n airflow #airflow webserver의 SVC 즉 service 이름을 찾아 냅니다.

NAME                    TYPE        CLUSTER-IP      EXTERNAL-IP   PORT(S)             AGE
airflow-postgresql      ClusterIP   10.43.30.119    <none>        5432/TCP            21h
airflow-postgresql-hl   ClusterIP   None            <none>        5432/TCP            21h
airflow-redis           ClusterIP   10.43.220.26    <none>        6379/TCP            21h
airflow-statsd          ClusterIP   10.43.112.58    <none>        9125/UDP,9102/TCP   21h
airflow-triggerer       ClusterIP   None            <none>        8794/TCP            21h
airflow-webserver       ClusterIP   10.43.133.204   <none>        8080/TCP            21h
airflow-worker          ClusterIP   None            <none>        8793/TCP            21h

#포트포워딩을 통해서 내부에만 존재하는 Airflow의 포트를 노출시켜 줍니다. 
$ kubectl port-forward svc/airflow-webserver 8080:8080 -n airflow
```

![img](airflow_asset/img11.png)



웹브라우저를 열어서 localhost:8080 으로 접속을 합니다.

![img](airflow_asset/img12.png)
...
git sync 시 지정한 GitLab 특정경로(http://gitlab.ssongman.duckdns.org/cjs/git-sync.git)

`airflow/dags/${USER_IDENTITY}` 디렉토리에 `tutorial_bash.py` 파일을 생성하고, 작성을 시작한다.
...
Airflow CLI로 방금 만든 DAG가 잘 반영되었는지 확인해보자. 원래는 `airflow list_dags` 명령어로 Airflow에 등록된 DAG 목록을 출력할 수 있다 

CLI Command는 `airflow dags list`로 아래와 같이 실행하면 된다.

```bash
$ kubectl get pod -n airflow
NAME                                 READY   STATUS    RESTARTS        AGE
airflow-postgresql-0                 1/1     Running   0               21h
airflow-redis-0                      1/1     Running   0               21h
airflow-scheduler-55f576bf5c-hckfh   3/3     Running   0               21h
airflow-statsd-7d985bcb6f-72gj6      1/1     Running   0               21h
airflow-triggerer-0                  3/3     Running   0               21h
airflow-webserver-75b6cd4f64-p9hsh   1/1     Running   0               21h
airflow-worker-0                     3/3     Running   0               21h
...
==============================+============================================+=========+=======
cjs_tutorial_bash             | airflow00/tutorial_bash.py                 | airflow | True
...
...
![image-20231019003447245](airflow_asset/image-20231019003447245.png)
...
CLI Command는 `airflow dags unpause [DAG ID]`로 아래와 같이 실행하면 된다.

```bash
$airflow dags unpause cjs_tutorial_bash
Dag: cjs_tutorial_bash, paused: False
```

Web UI에서 확인하면 ‘Off’였던 상태가 ‘On’으로 변경되고, DAG가 실행되고 있는 것을 볼 수 있다.

![image-20231019003931084](airflow_asset/image-20231019003931084.png)
